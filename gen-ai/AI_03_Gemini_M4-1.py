...
logging.basicConfig(
    ...
    filename='tp-report-summary.log',
    filemode='a'
)
...
# Function to check if the bucket is locked
def check_bucket_lock(s3, INPUT_BUCKET, LOCK_KEY):
    try:
        # Attempt to retrieve metadata of the lock key object
        s3.head_object(Bucket=INPUT_BUCKET, Key=LOCK_KEY)
        # If head_object does not raise an exception, the lock key exists (bucket is locked)
        return True
    except ClientError as e:
        # Handle the specific error code 404 (Not Found)
        if e.response['Error']['Code'] == '404':
            return False  # Lock key does not exist (bucket is not locked)
        else:
            logger.error(f"Error checking bucket lock: {e}. Exiting program.")
            sys.exit(1)  # Exit the program with an error code for other errors
        
# Function to acquire a lock
def acquire_lock(s3, INPUT_BUCKET, LOCK_KEY):
    max_attempts = 3
    attempt = 0
    while attempt < max_attempts:
        attempt += 1
        try:
            s3.put_object(Bucket=INPUT_BUCKET, Key=LOCK_KEY, Body=b'')
            ...
            if e.response['Error']['Code'] == 'NoSuchBucket':
                ...
                logger.error("Access denied to the bucket.")
            elif e.response['Error']['Code'] == 'SlowDown':
                logger.warning("Throttled by S3, retrying...")
                time.sleep(5)  # Wait before retrying
            else:
                logger.error(f"Unexpected error: {e}")
    
    logger.error(f"Failed to acquire lock after {max_attempts} attempts.")
    return False  # Assume cannot lock after max attempts

# Function to release the lock
def release_lock(s3, INPUT_BUCKET, LOCK_KEY):
    try:
        s3.delete_object(Bucket=INPUT_BUCKET, Key=LOCK_KEY)
...
try:
    ...
    logger.error("API key JSON file not found or . Exiting program.\n")
    sys.exit(1)
...
def generate_summary(json_input, prompt, retries=3, delay=5):
    ...
    for attempt in range(retries):
        ...
        except (google.api_core.exceptions.ResourceExhausted, google.api_core.exceptions.DeadlineExceeded, google.api_core.exceptions.InternalServerError) as e:
...
def main():
    ...
    # Lock file path in S3
    LOCK_KEY = f'{INPUT_PREFIX}lock'

    # Check if the bucket is locked
    check = check_bucket_lock(s3, INPUT_BUCKET, LOCK_KEY)

    if check:
        logger.info("Bucket is locked. Exiting program.\n")
        sys.exit(0)
    else:
        logger.info("Bucket is not locked. Continue.")
    
    # Try to acquire the lock
    if not acquire_lock(s3, INPUT_BUCKET, LOCK_KEY):
        logger.info("Error locking the bucket. Exiting program.")
        sys.exit(1)
    ...
    try:
        ...
        if check_bucket(response):
            ...
            release_lock(s3, INPUT_BUCKET, LOCK_KEY)
        ...
        sorted_files = sorted(files, key=lambda x: response['Contents'][files.index(x)]['LastModified'])

        # Process only the first 225 JSON files in the sorted list
        json_files_to_process = [key for key in sorted_files[:3] if key.endswith('.json')]

        # Process only the first 225 files object in the sorted list
        for index, key in enumerate(json_files_to_process):
            ...
            # Log the read file
            logger.info(f"Reading file {index+1}: {key}...")
